...
{!! Form::open(array('url' => url('industry-re-registration/local-machinery/store'),'method' => 'post', 'class' => 'form-horizontal', 'id' => 'local_machinery', 'enctype' =>'multipart/form-data', 'files' => 'true')) !!}

{!! Form::hidden('machine_id', \App\Libraries\Encryption::encodeId($localMachine->id) ,['class' => 'form-control input-md', 'id'=>'machine_id']) !!}
{!! Form::hidden('app_id', \App\Libraries\Encryption::encodeId($localMachine->app_id) ,['class' => 'form-control input-md', 'id'=>'app_id']) !!}

<div class="modal-header">
    <button type="button" class="close close_modal" data-dismiss="modal">&times;</button>
    <h4 id="local_machine_modal_head" class="modal-title"
        style="color: #452A73; font-size: 14px">{!!trans('CompanyProfile::messages.locally_collected')!!}</h4>
...
<div class="modal-body">
    ...
    <table id="local_machine_manual_input_table"
        ...
        <tr class="text-center">
            <td>
                {!! Form::text('local_machinery_nm', $localMachine->machinery_nm, ['class' => 'form-control input-md', 'placeholder'=> trans('CompanyProfile::messages.machinery_name'), 'id'=> 'local_machinery_name']) !!}
                {!! $errors->first('local_machinery_nm','<span class="help-block">:message</span>') !!}
            </td>
            <td>
                {!! Form::text('local_machinery_qty', $localMachine->machinery_qty, ['class' => 'form-control input-md onlyNumber input_ban', 'placeholder'=> trans('CompanyProfile::messages.n_number'), 'id'=> 'local_machinery_number']) !!}
                {!! $errors->first('local_machinery_qty','<span class="help-block">:message</span>') !!}
            </td>
            <td>
                {!! Form::text('local_machinery_price', $localMachine->machinery_price, ['class' => 'form-control input-md onlyNumber input_ban', 'placeholder'=> trans('CompanyProfile::messages.price_taka'), 'id'=> 'local_machinery_price']) !!}
