                            ...
                            <div class="col-sm-11 mb-2">
                                <form class="form-group">
                                    <input class="form-control mr-sm-2 search_field" type="search"
                                           name="request_data" id="request_data"  placeholder="হজযাত্রীর  ট্র্যাকিং নম্বর প্রদান করুন" autocomplete="off">
                                </form>
                            </div>
                            <div class="col-sm-1">
            ...
            <div class="src-hajj-tracking-number">
                <div class="dash-list-table my-4 toggle_data_talbe" style="display:none;" >
                    <span  class="btn btn-primary btn-sm mb-2">
                        একই ইউনিট এ সর্বমোট হজযাত্রী <span class="badge badge-light" id="total_pilgrim">0 জন </span>
                    </span>
                    <span  class="btn btn-secondary btn-sm mb-2" style="float: right;">
                        মোট কোটা <span class="badge badge-light" id="guide_capacity">0 জন </span>
                    </span>

                    <span  class="btn btn-info btn-sm mb-2">
                        অবশিষ্ট এড করতে পারবেন  <span class="badge badge-light" id="seat_available">0 জন </span>
                    </span>
...
<script>
...

    {
        ...
        var is_crm_guide = "<?php echo \Illuminate\Support\Facades\Auth::user()->is_crm_guide ?>";
                ...
                success: function(response) {
                    ...
                    if (response.data.data.hmisPilgrinmData == undefined || response.data.data.hmisPilgrinmData.length == 0 ) {
                        ...
                        for(var i =0 ; i<dataLen; i++){
                            ...
                            if(response.data.data.hmisPilgrinmData[i]['guide_id'] != 0 || response.data.data.hmisPilgrinmData[i]['will_not_perform'] != 0 ||  response.data.data.hmisPilgrinmData[i]['crm_tracking_no']!=null ||  response.data.data.hmisPilgrinmData[i]['hfr_tracking_no']!=null  ){
                                var status_msg = "";
                                if(response.data.data.hmisPilgrinmData[i]['will_not_perform'] != 0){
                                    status_msg = "Will Not Perform Hajj";
                                }else if(response.data.data.hmisPilgrinmData[i]['will_not_perform'] == 0 && response.data.data.hmisPilgrinmData[i]['crm_tracking_no'] != null || response.data.data.hmisPilgrinmData[i]['hfr_tracking_no'] != null){
                                    status_msg = "CRM Pilgrim";
                                }else{
                                ...
                                $('.load_pilgrim').append(
                                    '<tr id="row' + i + '"><td scope="row">'+  '<input type="checkbox" name="check_data['+response.data.data.hmisPilgrinmData[i]['pid']+']" value="'+response.data.data.hmisPilgrinmData[i]['pid']+'" class="checkbox">' + '</td>'+
                                    '<td>'+  '<input type="hidden" class="form-control" id="pid'+i+'" name="pid[]" value="'+response.data.data.hmisPilgrinmData[i]['pid']+'" readonly/>'+response.data.data.hmisPilgrinmData[i]['pid'] + '</td>'+
                                    '<td>'+
