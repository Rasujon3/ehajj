...
const handleSaveCropImg = () => {
    uploadCropArea.croppie('result', {
        ...
        imagePreview.value.profile = resp
        pilgrimData.value.pilgrim_img = resp
        closeModal();
    });
    uploadCropArea.croppie('destroy');
}
...
const submitForm = async () => {
    try {
        ...
        const response = await axios.post('/pilgrim/pre-reg/store', form, {
            headers: {
        ...
        if (response.data.status === true) {
            toastr.success('Pilgrim information store successfully.', '', {
                positionClass: 'toast-top-right',
            });
...
const getConfigNidPassport = async () => {
    try{
        const response = await axios.get(`/pilgrim/pre-reg/configuration-verification/nid-passport`);
...
const getDistrictPoliceStation = async (post_code,flag) => {
    try {
        if (post_code) {
            ...
            const response = await axios.get(`/pilgrim/pre-reg/get-district-police-station-by-postCode/${post_code}`);
            if(flag === 'permanent'){
...
const passportVerification = async () => {
    try{
        ...
        const response = await axios.post(`/pilgrim/pre-reg/passport-verification`,postData);
        if(response.data.status === true){
...
const checkDuplicatePilgrim = async () => {
    ...
    const response = await axios.post(`/pilgrim/pre-reg/check-duplicate-pilgrim`,postData);
    if(response.data.status === true){
        if(pilgrimData.value.identity === 'PASSPORT') {
            pilgrimData.value.name_en = response.data.data.passportData.full_name_english;
            pilgrimData.value.occupation = response.data.data.passportData.occupation;
...
const accountType = () => {
    if (pilgrimData.value.refund_account_type === 'Pilgrim own') {
        pilgrimData.value.refund_account_name = pilgrimData.value.name_en;
    }
};
        ...
        <div class="card-body">
            ...
            <form>
                ...
                <div v-if="currentStep === 4">
                    ...
                    <div class="row form-group">
                        ...
                        <div :class="['col-md-7', allerros.refund_account_type ? 'has-error' : '']">
                            <select v-model="pilgrimData.refund_account_type" @change="accountType" class="form-control input-sm">
                                <option value="">Select one</option>
                    ...
                    <div class="row form-group align-items-center" v-if="pilgrimData.identity !== 'PASSPORT'">
                        ...
                        <div :class="['col-md-7', allerros.pilgrim_img ? 'has-error' : '']">
                            <div class="row align-items-end">
                                ...
                                <button v-if="imagePreview.profile" type="button" class="btn btn-warning mr-2" @click="handleResetImage()"><i class="fas fa-times"></i> Reset Image</button>
                                <label v-if="!imagePreview.profile" class="btn btn-primary btn-file mb-0">
                                    <i class="fas fa-image"></i> Browse
                                    <input type="file" id="pilgrim_img" ref="pilgrim_img" class="form-control custom-file-input" @change="onFileChange" />
                                </label>
                            </div>
                    ...
                    <template v-if="pilgrimData.identity === 'DOB'">
                        <div class="row form-group">
                            ...
                            <div class="col-md-1" style="font-weight: bold;padding-top: 5px">:</div>
                            <div :class="['col-md-7', allerros.pilgrim_img ? 'has-error' : '']">
                                <input type="file" id="dob_img" ref="dob_img" class="form-control" @change="onDobImgChange" />
                                <span v-if="allerros.pilgrim_img" class="text-danger">{{ allerros.pilgrim_img[0] }}</span>
                                <span class="help-block">[File Format: *.jpg/ .jpeg/.png/ .gif | Max size 1 MB]</span>
                            </div>
                        </div>
                ...
                <div v-if="currentStep === 5" class="buttons-container float-right">
                    <button v-if="!loadingNext" @click.prevent="submitForm" class="btn btn-primary ">সাবমিট</button>
                    <button v-else class="btn btn-primary" disabled>অপেক্ষা করুন...</button>
                </div>
