...
const id = ref(null);
const encodeTrackingNo = ref(null);
const indexData = ref({
    occupationList : '',
    districtList : '',
    bankList : '',
...
const setPilgrimData = (pilgrim) => {
    ...
    pilgrimData.value.reg_payment_status = pilgrim.reg_payment_status;
    pilgrimData.value.reg_voucher_id = pilgrim.reg_voucher_id;
...
const fetchPilgrimDataFromApi = async (pilgrimId) => {
    ...
    const response = await axios.post(`/registration/get-pilgrim-data`, postData);
    ...
    encodeTrackingNo.value = response.data.encodeTrackingNo;
...
<template>
    <div class="card-body">
        <div class="preview-container">
            ...
            <div class="row">
                <div class="buttons-container float-right" v-if="pilgrimData.reg_payment_status === 0 && pilgrimData.reg_voucher_id === 0 && (pilgrimData.user_code == pilgrimData.reg_created_by)">
                    <router-link :to="{ name: 'EditRegPilgrim', params: { id: id, trackingNo: encodeTrackingNo } }"
