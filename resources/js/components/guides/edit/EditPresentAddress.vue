...
watch(() => guideData.value.present_division_id, (newPresentDivisionId) => {
    if(!presentDivisions || presentDivisions.value.length === 0){
        //console.log("presentDivisions is empty or not loaded yet.");
        return;
    }
...
watch(() => guideData.value.district_id, (newPresentDistrictId) => {
    ...
    if (selectedPresentDistrict != undefined) {
...
watch(() => guideData.value.thana_id, (newPresentThanaId) => {
    if (!presentPoliceStation.value || presentPoliceStation.value.length === 0) {
        //console.log("presentPoliceStation is empty or not loaded yet.");
        return;
    }
