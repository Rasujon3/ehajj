...
watch(() => guideData.value.permanent_division_id, (newPermanentDivisionId) => {
    if(!permanentDivisions || permanentDivisions.value.length === 0){
        //console.log("permanentDivisions is empty or not loaded yet.");
        return;
    }
...
watch(() => guideData.value.per_district_id, (newPermanentDistrictId) => {
    ...
    if (selectedPermanentDistrict != undefined) {
...
watch(() => guideData.value.per_thana_id, (newPermanentThanaId) => {
    if(!permanentPoliceStation || permanentPoliceStation.value.length === 0){
        //console.log("permanentPoliceStation is empty or not loaded yet.");
        return;
    }
