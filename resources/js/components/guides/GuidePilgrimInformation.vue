...
let isLocked = ref(0);
let pdfLink = ref('');
const getGuideId = computed(() => store.getters.getGuideId);
const guideId = getGuideId.value;
...
const voucherRowDetailsData = async () => {
    try {
        ...
        alert('Something went wrong!!! [GPI:004]');
    }
}
...
const getAlreadyAddedVoucherList = async () => {
    try {
        ...
        alert('Something went wrong!!! [GPI:005]');
    }
}
const addVoucherModalData = async () => {
    try {
        ...
        alert('Something went wrong!!! [GPI:006]');
    }
}
const addVoucherToGuide = async () => {
    try {
        ...
        alert('Something went wrong!!! [GPI:007]');
    }
}
...
const getVoucherAddedPilgrims = async () => {
    try {
        ...
        alert('Something went wrong!!! [GPI:008]');
    }
}
const locked = async (flag) => {
    try {
        ...
        alert('Something went wrong!!! [GPI:009]');
    }
}
...
const deleteVoucher = async (id) => {
    if (confirm('Are you sure you want to delete this voucher?')) {
        try {
            ...
            alert('Something went wrong!!! GPI:001');
        }
    }
};
...
const pdfGenerate = async () => {
    try {
        ...
        alert('Something went wrong!!! GAL:002');
        $("#generateVoucher").prop('disabled', false);
    }
};
...
const submitGuideRequest = async () => {
    try {
        ...
        $("#previous-click").prop('disabled', true);
        const response = await axios.post(`/guides/submit-guide-request/${guideId}`);

        if (response.data?.responseCode === 1) {
            alert(response.data?.msg);
            $("#submitGuideRequest").prop('disabled', false);
            $("#previous-click").prop('disabled', false);
        ...
        alert('Something went wrong!!! GPI:003');
        $("#submitGuideRequest").prop('disabled', false);
        $("#previous-click").prop('disabled', false);
    }
};
...
const openApplication = async (id) => {
    try {
        const response = await axios.get(`/guides/get-guide-application/${id}`);
        if (response.data?.responseCode === 1) {
            const guideData = response.data?.data[0][0];
            const voucherData = response.data?.data[1];
            const profilePic = response.data?.data[2];
            const ssc_certificate_link = response.data?.data[3];
            const hsc_certificate_link = response.data?.data[4];
            const honours_certificate_link = response.data?.data[5];
            const masters_certificate_link = response.data?.data[6];
            await store.dispatch('updateEditGuideInfo', guideData);
            await store.dispatch('updateVoucherInfo', voucherData);
            await store.dispatch('updateProfilePic', profilePic);
            await store.dispatch('updateSscCertificateLink', ssc_certificate_link);
            await store.dispatch('updateHscCertificateLink', hsc_certificate_link);
            await store.dispatch('updateHonoursCertificateLink', honours_certificate_link);
            await store.dispatch('updateMastersCertificateLink', masters_certificate_link);
            await router.push({ name: 'GuideEdit'});
        }
    } catch (error) {
        alert('Something went wrong!!! GE:001');
    }
};
const closeModal = () => {
    const modal = document.getElementById('pilgrimAddVoucher');
                ...
                <div class="border-card-block">
                    ...
                    <div class="bd-card-content">
                        <div class="border-card-block plg-member-list-card">
                            ...
                            <div class="bd-card-content" v-if="voucherInfo.length > 0">
                    ...
                    <div class="bd-card-footer">
                        <div class="flex-space-btw info-btn-group">
                            <button class="btn btn-default" id="previous-btn" type="button" @click="openApplication(guideId)"><span>Previous</span></button>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <div class="form-select2-item">
                            <select class="form-control hajj-select2" v-model="guideData.hajj_packages" :class="[allerros.hajj_packages ? 'has-error' : '']">
